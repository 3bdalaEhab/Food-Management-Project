...
export function DeleteConfirmation({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent showClose={false} className="max-w-xl bg-transparent border-none p-0 overflow-visible shadow-none">
                <motion.div
                    initial={{ opacity: 0, scale: 0.9, y: 30 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.9, y: 20 }}
                    className="w-full overflow-hidden rounded-[3rem] border border-[var(--border)] bg-[var(--sidebar-background)] shadow-[0_64px_128px_-32px_rgba(0,0,0,0.5)]"
                >
