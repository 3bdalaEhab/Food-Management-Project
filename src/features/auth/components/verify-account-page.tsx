...
import { ShieldCheck, Mail, ArrowRight, Loader2, KeyRound, Sparkles, Zap, ArrowLeft } from "lucide-react";
            ...
            >
                ...
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-max">
                    <div className="flex items-center gap-2 px-4 py-2 bg-white/5 backdrop-blur-3xl rounded-full border border-white/10 shadow-2xl">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/50">Account Activation Protocol</span>
                    </div>
                </div>
                ...
                <div className="glass-card rounded-[4rem] p-10 md:p-16 shadow-[0_64px_128px_-32px_rgba(0,0,0,0.8)] border border-white/10 backdrop-blur-[60px] bg-white/[0.03]">
                    <div className="flex flex-col items-center">
                        ...
                        <motion.div
                            ...
                            className="w-24 h-24 bg-gradient-to-br from-neutral-800 to-neutral-900 rounded-[2.5rem] flex items-center justify-center shadow-2xl border border-white/10 mb-10"
                        >
                        ...
                        <div className="text-center space-y-4 mb-12">
                            <h1 className="text-4xl font-black text-white tracking-tighter leading-none">
                                Identity <span className="text-primary-500 text-5xl">Verify</span>
                            </h1>
                            <p className="text-white/40 font-bold tracking-tight flex items-center gap-2 justify-center">
                                <Sparkles size={16} className="text-primary-500" />
                                We've transmitted a code to your terminal
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-8">
                            <div className="space-y-6 text-left">
                                <div className="space-y-2">
                                    <label className="text-[11px] font-black text-white/30 uppercase tracking-[0.2em] ml-1">Target Email</label>
                                    <div className="group relative">
                                        <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20" size={20} />
                                        <input
                                            {...register("email")}
                                            placeholder="agent@culinary.io"
                                            className="premium-input bg-white/[0.03] border-white/10 text-white h-16 pl-14 opacity-50 cursor-not-allowed"
                                            readOnly={!!email}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[11px] font-black text-white/30 uppercase tracking-[0.2em] ml-1">Universal Key</label>
                                    <div className="group relative">
                                        <KeyRound className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-primary-500 transition-colors" size={20} />
                                        <input
                                            {...register("code")}
                                            placeholder="••••"
                                            className="premium-input bg-white/[0.03] border-white/10 text-white h-16 pl-14 text-center tracking-[1em] font-black text-2xl"
                                            maxLength={4}
                                        />
                                    </div>
                            ...
                            >
                                ...
                                {isPending ? <Loader2 className="animate-spin w-8 h-8" /> : (
                                    <div className="flex items-center gap-3">
                                        <span className="font-black italic uppercase tracking-tighter">Initialize Identity</span>
                        ...
                        <div className="mt-14 pt-10 border-t border-white/5 w-full text-center">
                            <p className="text-sm font-bold text-white/30">
                                Protocol failed? <button className="text-primary-500 font-black uppercase hover:text-primary-400 transition-colors ml-1">Resend Transmission</button>
                            </p>
                            <Link to="/login" className="inline-flex items-center gap-2 mt-6 text-xs font-black text-white/20 hover:text-white uppercase tracking-widest transition-colors group">
