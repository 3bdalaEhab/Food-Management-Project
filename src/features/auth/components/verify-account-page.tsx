                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="relative group">
                                    <Mail className="absolute start-3.5 sm:start-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-[var(--muted-foreground)]/50 z-10 pointer-events-none" aria-hidden="true" />
                                    <input
                                        {...register("email")}
                                        type="email"
                                        placeholder="you@example.com"
                                        readOnly={!!email}
                                        className="w-full h-12 sm:h-14 bg-[var(--background)]/60 border border-[var(--border)] rounded-xl sm:rounded-2xl ps-11 sm:ps-12 pe-4 text-sm sm:text-base font-medium placeholder:text-[var(--muted-foreground)]/40 focus:bg-[var(--background)] focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all outline-none disabled:opacity-50 cursor-not-allowed"
                                    />
                                </div>
                                {errors.email && (
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="relative group">
                                    <KeyRound className="absolute start-3.5 sm:start-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-[var(--muted-foreground)]/50 group-focus-within:text-primary-500 transition-colors z-10 pointer-events-none" aria-hidden="true" />
                                    <input
                                        {...register("code")}
                                        placeholder="••••"
                                        maxLength={4}
                                        className="w-full h-14 sm:h-16 bg-[var(--background)]/60 border border-[var(--border)] rounded-xl sm:rounded-2xl ps-11 sm:ps-12 pe-4 text-center text-2xl sm:text-3xl font-black tracking-[0.5em] placeholder:text-[var(--muted-foreground)]/40 focus:bg-[var(--background)] focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all outline-none"
                                    />
                                </div>
                                {errors.code && (
                                    <p className="text-[10px] sm:text-xs text-red-500 font-semibold ms-1">{errors.code.message}</p>
                                )}
                            </div>
