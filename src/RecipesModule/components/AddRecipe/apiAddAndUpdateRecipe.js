...
export function appendToFormData(values) {
 ...
 export async function apiCreateNewRecipe(formData) {
    ...
    try {
      const { data } = await axios.post(`https://upskilling-egypt.com:443/api/v1/Recipe/`, formData, { headers: { Authorization: token } });
      console.log(data);
...
export async function apiUpdateRecipeItem({formData,id}) {
    ...
    let token = localStorage.getItem("token");
    try {
      const { data } = await axios.put(`https://upskilling-egypt.com:443/api/v1/Recipe/${id}`, formData, { headers: { Authorization: token } });
      console.log(data);
      toast("Updated", { icon: "😊" });
 ...
 export async function apiTag() {
    let token = localStorage.getItem("token");
    try {
      const { data } = await axios.get(`https://upskilling-egypt.com:443/api/v1/tag/`, { headers: { Authorization: token } });
      return data
    } catch (error) {
      console.log(error);
    }
 ...
 export async function apiGetItem(id) {
    let token = localStorage.getItem("token");
    try {
      const { data } = await axios.get(`https://upskilling-egypt.com:443/api/v1/Recipe/${id}`, { headers: { Authorization: token } });
     return data
    } catch (error) {
      console.log(error);
    }
